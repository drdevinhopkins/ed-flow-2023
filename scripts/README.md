...
`forecast_intraday_daily_inflow.py` is the guarded hourly-workflow companion. It uses the
shared `intraday_day_completion_model.py` implementation and writes a separate one-row
`intraday-daily-inflow-forecast.csv` plus `intraday-daily-inflow-status.json`. It does not
modify `forecast-v2.1.csv`, and invalid, incomplete, stale, or out-of-window inputs are
suppressed rather than imputed.

...
def main() -> None:
    ...
    required = {
        "target_name", "horizon_band", "n", "n_issue_dates", "n_unique_target_hours",
        "prospective_span_days", "promotion_status"
    }
    ...
    result = frame[[
        "target_name", "horizon_band", "n", "n_issue_dates", "n_unique_target_hours",
        "prospective_span_days", "directional_criteria_met", "promotion_evidence_ready",
        "promotion_status"
    ]].copy()
    ...
    result["readiness_fraction_dates"] = (
        result["n_issue_dates"] / MIN_ISSUE_DATES
    ).clip(upper=1)
    ...
    result["overall_readiness_fraction"] = result[[
        "readiness_fraction_rows", "readiness_fraction_dates",
        "readiness_fraction_unique_hours", "readiness_fraction_span"
    ]].min(axis=1)

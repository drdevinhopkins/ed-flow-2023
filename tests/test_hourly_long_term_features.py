...
from scripts.hourly_long_term_features import (
...
def test_annual_lag_matches_same_calendar_hour() -> None:
    ...
    assert np.isclose(frame.loc[0, "Total_TBS__lag_1y"], lookup.loc[pd.Timestamp("2024-02-10 13:00:00")])
    assert np.isclose(frame.loc[0, "Total_TBS__lag_2y"], lookup.loc[pd.Timestamp("2023-02-10 13:00:00")])
...
def test_future_secular_features_are_frozen_at_cutoff() -> None:
    ...
    timestamps = pd.Series(pd.date_range(cutoff - pd.Timedelta(hours=3), periods=28, freq="h"))
    frame = build_long_term_feature_frame(flow, timestamps, ["Total_TBS"], cutoff=cutoff)
    future = frame.loc[frame["ds"] > cutoff]
    cols = [c for c in future.columns if "__level_" in c or "__growth_90d_yoy" in c or "__growth_365d_yoy" in c]
    for column in cols:

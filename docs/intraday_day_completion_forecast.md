...
The model remains an experimental companion to the production hourly forecast. The
hourly v2.1 workflow can run it after the primary forecast has already been generated and
publish a separate `intraday-daily-inflow-forecast.csv`; it never adds rows to or changes
the schema of `forecast-v2.1.csv`. A separate status JSON records explicit suppression
when the source is stale, incomplete, invalid, or outside the 06:00–22:00 model window.
...
Publishing this separately labelled experimental companion does not authorize replacing
the primary operational forecast or incorporating it into decision-support messaging.
That promotion still requires a deterministic fallback, freshness monitoring, a
scoring/runbook path, and an explicit go/no-go review after prospective evidence is
available.

## Hourly workflow companion

The operational entrypoint is additive and can be run independently:

```bash
python scripts/forecast_intraday_daily_inflow.py \
  --weather-csv weather_backfilled.csv
```

On a valid cutoff it overwrites a one-row latest snapshot containing observed arrivals,
the predicted midnight total, P10/P90 bounds, expected additional arrivals, and the
prior-update baseline. Use `--upload-dropbox` only when the standard Dropbox credentials
are present. The model is retrospectively evaluated from 06:00 through 22:00; the
prospective evidence window is explicitly labelled as 11:00–18:00 in every output row.
